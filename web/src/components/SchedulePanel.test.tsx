...
describe('SchedulePanel', () => {
  it('renders nothing when there is no classified data at all', () => {
    const { container } = render(<SchedulePanel categories={[]} onIsolate={vi.fn()} onJumpTo={vi.fn()} />)
  ...
  it('shows counts per category and isolates + jumps on a row click', async () => {
    ...
    render(<SchedulePanel categories={categories} onIsolate={onIsolate} onJumpTo={onJumpTo} />)
