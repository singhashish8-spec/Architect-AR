...
| QR code generation | **`qrcode.react`** | Renders a project's shareable link as a QR code, printable on a physical drawing sheet — see [`../features/client-presentation-viewer.md`](../features/client-presentation-viewer.md). Chosen over the base `qrcode` package for a plain React component with no manual canvas/SVG wiring; renders SVG specifically (not canvas/PNG) so it stays crisp printed at any sheet size. |
